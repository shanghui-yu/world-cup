<template>
<transition name="fade">
  <div class="rule-pop">
    <div class="flex"></div>
    <div class="rule-main">
      <!-- 关闭 -->
      <div class="close"  @click="showRule"></div>
      <h3 class="title">活动规则</h3>
      <p>
        <em>玩法：</em>车型与队伍进行匹配，类似翻牌的形式；正面为对应车型，背面为与车型相匹配的队伍；只要玩法是一串一，二串一，三串一，翻完之后进行投注，压输赢；每人每轮最少可以翻1次，最多可以翻3次；如果都猜对，依次积分为2，5，10；每天分享活动增加积分1分。
      </p>
      <p>
        <em>规则：</em> 每次一共8张牌，用户挑选自己喜欢的牌点击进行翻转，背面是固定编组内随机的代表队；每人每轮最多可翻转3张牌，最少翻转1张牌;小组赛阶段翻转完成后每张牌的下面都会有两个按钮选项，“赢”“输”；可以组成1串1，2串1，3串1，选择完成点击确定按钮；淘汰赛阶段每次一共8张牌，每人每轮只有一次确认机会，用户挑选自己喜欢的牌点击进行翻转，背面是随机代表队，小礼品及小额积分；翻到球队依然进行投注，翻到其他直接进行领取。
      </p>
      <button class="know" @click="showRule">我知道了</button>
    </div>
  </div>
</transition>
</template>

<script>
export default {
  data () {
    return {
      msg: ''
    }
  },
  methods: {
    showRule () {
      this.$emit('showRule')
    }
  }
}
</script>

<style scoped lang="less">
  .rule-pop{
    position: fixed;
    z-index: 1000;
    top:0;
    width: 100%;
    height: 100%;
    left: 0;
    background: rgba(0, 0, 0, 0.69);
    display: flex;
    justify-content: center;
    flex-direction: column;
    -webkit-overflow-scrolling: touch;
    overflow-y: auto;
    z-index: 100;
  }
  .flex{
    flex: 1;
  }
  .rule-main{
    margin:150px 30px 0;
    position: relative;
    width: 690px;
    background: url('http://img5.168trucker.com/topic/images/worldCup/model-bg.png');
    height: 1058px;
    .close{
      width: 88px;
      height: 111px;
      position: absolute;
      top:-111px;
      right: 36px;
      background: url(data:image/png;base64,) no-repeat 50% 50%;
    }
    .title{
      margin:60px auto 30px;
      background: url('http://img5.168trucker.com/topic/images/worldCup/rule-title.png') 50% 50%;
      background-size: cover;
      width: 350px;
      height: 88px;
      font-size: 0;
    }
    p{
      padding: 0 55px 0 48px;
      color: #fff;
      font-size: 28px;
      line-height: 40px;
      text-align: justify;
      margin-top: 30px;
      em{
        font-size: 36px;
      }
    }
    .know{
      margin:30px auto;
      background: url('http://img5.168trucker.com/topic/images/worldCup/know.png') 50% 50%;
      background-size: cover;
      width: 320px;
      height: 96px;
      display: block;
      font-size: 0;
    }
  }
  .fade-enter-active, .fade-leave-active {
    transition: transform .5s;
     transform: translate(0, 0);
  }
  .fade-enter, .fade-leave-active {
    transform: translate(0, 100%);
  }
</style>
